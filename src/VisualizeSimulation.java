...
public class VisualizeSimulation extends JFrame {
     ...
     */
    public VisualizeSimulation(String searchAlgorithm, int start_row,
                               int start_col, int target_row, int target_col,
                               String env_id) {
...
class EnvironmentPanel extends JPanel {
    ...
    public EnvironmentPanel(Environment env, ArrayList<Robot> robots) {
        setPreferredSize(
            new Dimension(env.getCols() * TILESIZE, env.getRows() * TILESIZE));
        this.env = env;
        this.robots = robots;
        ...
        this.timer = new Timer(timeSteps, new ActionListener() {
            ...
            public void printPerformanceMeasure() {
                System.out.printf(
                    "Simulation Completed in %d timesteps with %d costs.\n",
                    timesteps, path_cost);
                for (Robot robot : robots) {
